...
Required JSON shape per exchange:
    ...
    hyperliquid_main:  {"exchange", "private_key", "wallet_address",
                        "vault_address"?}
"""
...
_REQUIRED_FIELDS: dict[str, set[str]] = {
    ...
    "bybit": {"key", "secret"},
    "okx": {"key", "secret", "passphrase"},
    # Phase 1 Stage 2: EIP-712 (Hyperliquid). `vault_address` is optional
    # and not in the required set; the enclave deserialises it as
    # `Option<String>` via `#[serde(default)]`.
    "hyperliquid_main": {"private_key", "wallet_address"},
}
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Exchange the secrets are for (kucoin/binance/binance_futures/bybit/okx/hyperliquid_main)",
    )
    parser.add_argument(
        "--kms-key",
        required=True,
