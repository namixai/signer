...
- **Tamper-evident builds:** the reproducible-build pins that fix the enclave PCR0 live in `poc/policies/build-pins.txt`, and the KMS key policy is what binds the PCR0 / `ImageSha384` the enclave is allowed to attest with (illustrative policy snapshots: `poc/policies/kms-policy-day{1,2,3}-*.json`). A live KMS policy attesting a PCR0 that is not derivable from the pinned reproducible build is by definition unauthorized. (The Terraform that provisions the production KMS policy is not included in this public tree — request it for a full attestation-integrity review.)
...
| A2 — Backdoored gateway | No (can be a signing oracle, scoped by UPL) | UPL enforces action/venue/path enclave-side (live); withdraw/transfer denied on every path; order-size caps on structured order paths only — generic sign path is deny-withdraw-bound, not cap-bound |
