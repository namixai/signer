...
**Status:** production build, **multi-tenant**; mainnet venue keys exist for the operator's own funds only (no third-party money in production, no external audit). The production enclave's measurement is **not named in this file** — read it from `/attestation` (`pcr0_sha384`); every deployed measurement is **reproducible from this repository at a tag `pcr0-<prefix>`** (check the tag out first; `main` carries later changes and builds to a different, undeployed PCR0 — see [Reproducible build](../README.md#reproducible-build) and [`docs/VERIFY-SIGNER-YOURSELF.md`](../docs/VERIFY-SIGNER-YOURSELF.md)). The attestation registry contract is live on Base mainnet; ask it `isPCR0Active(bytes)` about the value you read — the 96-hex `pcr0_sha384` goes in `0x`-prefixed, because the call takes 48 **raw bytes** and the hex text would be 96 of them (`InvalidPCR0Length`) — and compare the **owner** to `0x21538eBF…`, because `registerPCR0` is permissionless. The public **demo** box rotates on its own schedule — read what it attests from its own `/attestation` rather than from this line, because the two boxes can be on different measurements in any rotation window. `/attestation` no longer carries a `registered_onchain` field: it was a boolean this gateway computed about a fact with a public oracle, so believing it bought you nothing over believing us. Note that the registry keeps ONE active measurement per owner: registering the production measurement deprecates the demo's in the same transaction, so the two boxes cannot both answer `true` while they run different images.
...
2. The attestation registry contract is live on Base mainnet (`0x38b42eED740b0fDeb211bBDf773F2238cAEec240`) and holds the production PCR0 as a public record — one active measurement per owner, so each rotation's registration deprecates the previous one. Read it with `isPCR0Active(bytes)`, passing the measurement as `0x` + the 96-hex `pcr0_sha384` (48 raw bytes; see [DEMO.md](../DEMO.md) for the exact command), and compare the returned owner to `0x21538eBF6598e5866BA496A954dE8E39097bFB59`: registration is permissionless, so `active=true` alone proves nothing. Read it with `isPCR0Active(bytes)` and compare the returned owner to `0x21538eBF6598e5866BA496A954dE8E39097bFB59`: registration is permissionless, so `active=true` alone proves nothing.
...
```
The reproducible enclave image (EIF → PCR0) is built on an EC2 build host (Docker + `nitro-cli`) in a pinned musl container; local native builds only confirm the source compiles + tests pass. Build the enclave image with **`SIGNER_REQUIRE_POLICY=1 ./scripts/build-eif.sh`** — the strict/money-path build the **mainnet/production** enclave runs; on tag `pcr0-fbaad62f` its PCR0 reproduces `fbaad62f826c1451c51b03edb9b87b319bcff651d3283c24a24540899662dddc0c331e34cdd0315cbaf4c6aa05a2831f` (measured 2026-09-10 from two separate clean anonymous clones, the second gated by `scripts/reproducibility-from-public-clone.sh` with verdict `REPRODUCIBLE`, nitro-cli 1.4.4; on the production lane since 2026-09-10 and on the demo lane since 2026-09-11); the previous tags `pcr0-60036cd3` (production 2026-09-03 → 2026-09-10) and `pcr0-103ccd79` (demo until 2026-09-11) are both deregistered on-chain. Which of them an endpoint attests today is that endpoint's `/attestation` to say. HEAD of `main` carries changes merged after the latest tag and builds to a different, undeployed PCR0 — see the repository README's "Reproducible build" table and `scripts/enclave-closure-check.py`. The **public demo** endpoint is a separate box on its own rotation schedule, and this file deliberately does not name what it measures — read that from the demo box's own `/attestation`, which is the only source that cannot go stale. The two lanes have parted three times already: the demo box measured `32d25d8c…` between 2026-08-24 and 2026-08-27; they parted on 2026-09-03, when production moved to tag `pcr0-60036cd3` and the demo box stayed behind; and again on 2026-09-10, when production moved to `pcr0-fbaad62f` a day before the demo box followed. So do not read either tag above as "the demo box": check that endpoint against the measurement it reports itself. One published number never covers both lanes for long. See [`docs/VERIFY-SIGNER-YOURSELF.md`](../docs/VERIFY-SIGNER-YOURSELF.md) to verify the live enclave against it.
